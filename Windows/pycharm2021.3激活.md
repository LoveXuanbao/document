- 官网下载路径

```
https://www.jetbrains.com/pycharm/download/other.html
```

- 安装省略。。。
- 激活
- 安装完毕后询问是否重启狄电脑，选择下面得稍后重启，然后点击Finish。

![image-20221122115118663](https://niuzhan-1306014148.cos.ap-beijing.myqcloud.com/Typora/image-20221122115118663.png)

- 右键点击桌面图标，选择打开文件所在位置，找到目录下得pycharm64.ext.vmoptions 文件，打开
- 添加以下配置，路径是fineagent.jar文件得存放路径

```
-javaagent:C:\Program Files\JetBrains\PyCharm 2021.3\fineagent.jar
```

- 之后打开pycharm，第一次询问是否导入pycharm设置，选择下面得不导入，然后点击OK
- 弹出激活界面，选择用激活码，然后将文件末尾得激活码复制进去激活。

![image-20221122115323781](https://niuzhan-1306014148.cos.ap-beijing.myqcloud.com/Typora/image-20221122115323781.png)

- 激活码

```
5AYV1D1RE5---
```

